...
public class ItsChocolateTime {
    public static void main(String[] args) {
        ...
        double enoughChocolateProduced = chocolateTypes.values()
                .stream().filter(v -> v > 0).mapToDouble(Double::doubleValue).count();
        ...
        chocolateTypes.entrySet().stream().filter(e -> e.getValue() > 0)
                .forEach(e -> System.out.println(String.format("# %s --> %.0f", e.getKey(), e.getValue())));
